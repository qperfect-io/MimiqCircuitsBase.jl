...
function run_plotting(args)
    ...
    if !isfile(csv_path)
        if isfile(csv_name)
            ...
            error("CSV file not found: $csv_path. Run experiment first.")
        end
    end

    println("Loading basis comparison data from $csv_path...")
    df = CSV.read(csv_path, DataFrame)

    println("Regenerating plots in $output_dir...")
    generate_plots(df, output_dir; filename_prefix="sk_basis")
