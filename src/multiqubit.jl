...
"""
    struct GateCCX <: AbstractGate{3}

C₂X (or C₂NOT) 3-qubits gate. Where the first two qubits are used as controls.
"""
struct GateCCX <: AbstractGate{3} end

gatename(::Type{GateCCX}) = "CCX"
...
"""
    struct GateCSWAP <: AbstractGate{3} end

3-qubits control SWAP gate where the first qubit is the control.
"""
struct GateCSWAP <: AbstractGate{3} end

gatename(::Type{GateCSWAP}) = "CSWAP"
