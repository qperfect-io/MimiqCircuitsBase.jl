...
@doc raw"""
    struct Or{N} <: AbstractClassical{N}
...
julia> push!(Circuit(), Or(),1, 3, 4)
4-bit circuit with 1 instruction:
└── c[1] = c[3] | c[4]

julia> push!(Circuit(), Or(5), 1, 3, 5, 8, 13)
13-bit circuit with 1 instruction:
└── c[1] = c[3] | c[5] | c[8] | c[13]

julia> push!(Circuit(), Or(8), 1, 2, 3, 4, 5, 6, 7, 8)
...
struct Or{N} <: AbstractClassical{N}
    function Or(N)
        if N < 3
            throw(ArgumentError("Or operation requires at least 3 classical bits."))
        end
        new{N}()
    end
end
...
function Base.show(io::IO, ::MIME"text/plain", g::Or{N}) where {N}
    compact = get(io, :compact, false)
    if compact || N > 6
        print(io, "c[?1] = | @ c[?2:?N]")
    else
